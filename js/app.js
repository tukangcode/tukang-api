...
function statusBadge(p) {
  ...
  if (s === "manual") return `<span class="chip chip-manual">⚠ Online — dicek manual (situs tolak cek otomatis)</span>`;
  ...
  mount.innerHTML = `
    ...
    <div class="section">
      ...
      <table class="spec-table">
        ...
        <tr><td>Status website</td><td>${window.__providerStatus && window.__providerStatus.sites[p.id] === "manual" ? "Online — verifikasi manual (situs ini menolak pengecekan otomatis, jadi tidak diprobe sama sekali)." : `Dicek otomatis tiap 30 menit oleh GitHub Actions (1 request ringan per situs). ${window.__providerStatus ? "Cek terakhir: " + esc(new Date(window.__providerStatus.checked_at).toLocaleString("id-ID")) : "Belum ada hasil cek."} Ini status website saja, bukan ketersediaan model.`}</td></tr>
